...
namespace md5_stuff
{
	...
	void Encrypt(const unsigned char *input, size_t length, unsigned char *output)
	{
		// make a temp version of input with enough space for padding and len appended
		unsigned long extra_len = 64 - length % 64;
		if (extra_len <= 8)
		{
			extra_len += 64;
		}
		const std::vector<unsigned char> buffer(extra_len + length);
		unsigned char* temp = const_cast<unsigned char*>(buffer.data());

		// number of 16 word blocks
		const unsigned long N = (extra_len + length) / 64;

		const unsigned char* input2 = input;
		unsigned char* temp2 = temp;
		unsigned char* end = temp + length;
		...
		*temp2 = static_cast<unsigned char>(0x80);
		...
		unsigned long low = length & 0xFFFF;
		unsigned long high = length >> 16;
		unsigned long upper;
		unsigned long tmp;
		tmp = low * 8;
		low = tmp & 0xFFFF;
		...
		*temp2 = 0;
		...
		for (unsigned long i = 0; i < N; ++i)
		{
			...
			{
				x[j] = (
					(static_cast<uint32_t>(temp[4 * (j + 16 * i) + 3]) << 24) |
...
std::string MD5::Encrypt(const std::string &input)
{
	...
	md5_stuff::Encrypt(reinterpret_cast<const unsigned char*>(input.data()), input.size(), output);
