...
char BlockAllocator::s_block_size_lookup_[kMaxBlockSize + 1];
	...
	for (int i = 0; i < num_chunk_count_; ++i)
	{
		...
		if (chunk->block_size != block_size)
		{
			assert((char *)p + block_size <= (char *)chunk->blocks ||
				(char *)chunk->blocks + kChunkSize <= (char *)p);
		}
		else
		{
			if ((char *)chunk->blocks <= (char *)p && (char *)p + block_size <= (char *)chunk->blocks + kChunkSize)
			{
				found = true;
			}
		}
